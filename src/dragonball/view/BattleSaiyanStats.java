...
public class BattleSaiyanStats  extends JLayeredPane{
	
	public BattleSaiyanStats(String name, int level, int currHP, int maxHP, int currKi, int maxKi, int currStamina,
		...
		health.add(healthBar, 0);
		
		JLabel healthTextS = new JLabel(currHP + "/" + maxHP);
		healthTextS.setHorizontalAlignment(SwingConstants.LEFT);		
		healthTextS.setForeground(Color.black);
		healthTextS.setBounds(8, 3, healthText.getWidth(), healthText.getHeight());
		healthTextS.setFont(new Font(healthTextS.getFont().getName(), healthTextS.getFont().getStyle(), 30));
		healthBar.add(healthTextS);
